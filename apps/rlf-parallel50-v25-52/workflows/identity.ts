...
function vatCountry(id:string):string|null {
  const prefix=id.slice(0,2);
  return VAT_PREFIX_TO_COUNTRY[prefix]??(prefix==='EL'||prefix==='GR'?'GR':null);
}
...
function validRegistration(id:string,minDigits:number,maxLength=20):boolean {
  const digits=(id.match(/\d/g)?.length??0);
  return id.length>=5&&id.length<=maxLength&&digits>=minDigits&&!/^(?:0+|1+)$/.test(id)&&!/VATNUMBER|TAXNUMBER|COMPANYNUMBER|REGISTRATIONNUMBER/.test(id);
}
...
function registration(text:string,country:CountryDetection):RegistrationMatch|null {
  ...
  for(const item of patterns){
    ...
    if(validRegistration(id,item.minDigits)) return {keyPrefix:item.keyPrefix,id,countryCode:item.countryCode};
  }
  if(country.code&&country.code!=='NON_EU'){
    const generic=text.match(/\b(?:COMPANY|REGISTRATION)\s+NUMBER\s*[:#]?\s*([A-Z]{0,3}[-/ ]?\d[\d A-Z/-]{3,15}?)(?=\s*(?:[.;|]|VAT|TAX|$))/i);
    if(generic?.[1]){
      const id=compact(generic[1]);
      if(validRegistration(id,4,18)) return {keyPrefix:`${country.code}-REG`,id,countryCode:country.code};
    }
  }
  if(country.code==='PT'||country.code==='ES'){
    ...
    if(nif?.[1]){
      ...
      if(validRegistration(id,7,12)) return {keyPrefix:country.code==='PT'?'PT-NIF':'ES-NIF',id,countryCode:country.code};
    }
  }
  return null;
}
...
export function detectCountry(domain:string,legalText:string):CountryDetection {
  ...
  if(local?.countryCode) return {code:local.countryCode,basis:'LEGAL_COUNTRY'};
...
function contractingName(text:string){
  const lines=norm(text).split(/(?<=[.;|])\s+/).filter(Boolean);
  const line=lines.find(item=>LEGAL_FORM.test(item)&&!BAD_NAME_LINE.test(item)&&item.length>=5&&item.length<=180);
...
export function analyzeIdentity(domain:string,legalText:string,country:CountryDetection):IdentityAnalysis {
  ...
  const rawVat=country.code==='NON_EU'?null:findEuVat(text);
  const euVat=rawVat&&(!country.code||country.basis==='VAT'||vatCountry(rawVat)===country.code)?rawVat:null;
  const rawLocal=country.code==='NON_EU'?null:registration(text,country);
  const local=rawLocal&&(!country.code||rawLocal.countryCode===country.code)?rawLocal:null;
  ...
  else if(name&&addr&&!generic&&country.code&&country.code!=='NON_EU'&&(country.basis==='EU_TLD'||country.basis==='LEGAL_COUNTRY')){
    const key=compact(`${name}|${addr}`).slice(0,96);
    identityKey=key?`NAME-ADDRESS:${key}`:null;
    basis=identityKey?'NAME_ADDRESS':'NONE';
  }
  const named=NAMED_OPERATOR.test(text)||Boolean(name);
  const hasAddress=Boolean(addr)||(ADDRESS.test(text)&&/\b\d{1,5}\b/.test(text));
  const strong=text.length>=120&&named&&hasAddress&&(basis==='VAT'||basis==='REGISTRATION'||basis==='NAME_ADDRESS');
