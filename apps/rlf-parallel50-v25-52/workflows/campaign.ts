...
function finalize(campaignId:string,startedAt:string,completedAt:string,cycles:number,results:LaneCycleResult[]):CampaignResult {
  ...
  for(const result of results) for(const attempt of result.searchAttempts){
    const row=providerStats[attempt.name]??={attempts:0,http200:0,relevantLinks:0,challenges:0,errors:0,durationMs:0,bodyBytes:0,statusCounts:{},contentTypes:{},responseHashes:[]};
    row.attempts+=1;row.http200+=attempt.status===200?1:0;row.relevantLinks+=attempt.linkCount;row.challenges+=attempt.challenge?1:0;row.errors+=attempt.error?1:0;row.durationMs+=attempt.durationMs;row.bodyBytes+=attempt.bodyLength;
    const status=attempt.status===null?'null':String(attempt.status);row.statusCounts[status]=(row.statusCounts[status]??0)+1;
    const contentType=attempt.contentType??'unknown';row.contentTypes[contentType]=(row.contentTypes[contentType]??0)+1;
    if(attempt.responseHash&&!row.responseHashes.includes(attempt.responseHash)&&row.responseHashes.length<12)row.responseHashes.push(attempt.responseHash);
  }
