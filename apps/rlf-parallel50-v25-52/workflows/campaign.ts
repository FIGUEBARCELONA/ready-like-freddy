...
function collapseIdentityDuplicates(input:Candidate[]){
  ...
  return sorted.map(candidate=>{
    if(!candidate.identityKey||candidate.status==='DUPLICATE_KNOWN') return candidate;
