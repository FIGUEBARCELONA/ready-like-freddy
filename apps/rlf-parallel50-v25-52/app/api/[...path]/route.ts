...
const ONE_SHOT_SWEEP_HASH='b51b1c9dd5e222f28cc485e342a7291f1cd18759bcf0cc7f10696363c0f9cd73';
const MONITOR_RUN_ID='wrun_01M0Q8981CTB61S3DJJ06JZ2FW';
const LATEST_SWEEP_RUN_ID='wrun_01M0TCH7KDYYZCGYHEV43Y5C8F';
...
async function startSweep(){const id=campaignId('RLF-P50-SWEEP-V255515');const run=await start(parallel50Sweep,[{campaignId:id,cycle:9,maxCandidatesPerLaneCycle:8}]);return{campaignId:id,runId:run.runId};}
async function startMonitor(){const id=campaignId('RLF-P50-MONITOR-V255515');const run=await start(parallel50Campaign,[{campaignId:id,cycles:12,intervalMs:7200000,maxCandidatesPerLaneCycle:6}]);return{campaignId:id,runId:run.runId};}

export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  const parts=(await params).path??[];const path=parts.join('/');void request;
  const base={version:'25.55.15',workflowVersion:'4.8.4_PINNED',searchProfile:'EU27_FAIL_CLOSED_IDENTITY_V10',delta:'0051',dedupRegistry:CANONICAL_REGISTRY_COVERAGE,identityQuarantineCount:KNOWN_IDENTITY_QUARANTINE_DOMAINS.size,dependencyAudit:{moderate:0,high:0,critical:0,total:0}};
