...
const ONE_SHOT_SWEEP_HASH='f4048030bf75624f2efdaa1e69280262cd04591611794ac453f2d6ef0df55511';
const MONITOR_RUN_ID='wrun_01M0Q8981CTB61S3DJJ06JZ2FW';
const LATEST_SWEEP_RUN_ID='wrun_01M0T9ZY5Y14T9QJPY984YS38P';
...
async function startSweep(){const id=campaignId('RLF-P50-SWEEP-V25559');const run=await start(parallel50Sweep,[{campaignId:id,cycle:7,maxCandidatesPerLaneCycle:8}]);return{campaignId:id,runId:run.runId};}
async function startMonitor(){const id=campaignId('RLF-P50-MONITOR-V25559');const run=await start(parallel50Campaign,[{campaignId:id,cycles:12,intervalMs:7200000,maxCandidatesPerLaneCycle:6}]);return{campaignId:id,runId:run.runId};}

export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  const parts=(await params).path??[];const path=parts.join('/');void request;
  const base={version:'25.55.9',workflowVersion:'4.8.4_PINNED',searchProfile:'EU27_STRICT_IDENTITY_V8',delta:'0051',dedupRegistry:CANONICAL_REGISTRY_COVERAGE,identityQuarantineCount:KNOWN_IDENTITY_QUARANTINE_DOMAINS.size,dependencyAudit:{moderate:0,high:0,critical:0,total:0}};
