...
const SWEEP_BOOTSTRAP_HASH='6dbbe53b2ab3fd1a74c186e27d894518c2a0fb6406b1e8d27109fafb167e34d0';
...
async function startSweep() {
  const id=campaignId('RLF-P50-SWEEP-V2553');
  const run=await start(parallel50Sweep,[{campaignId:id,cycle:1,maxCandidatesPerLaneCycle:6}]);
  return {campaignId:id,runId:run.runId};
}

async function startMonitor() {
  const id=campaignId('RLF-P50-MONITOR-V2553');
...
export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}) {
  ...
  if(path==='health') return json({
    ok:true,version:'25.53.0',workflowVersion:'4.4.0_PINNED',executionBackend:'VERCEL_WORKFLOW',queue:'VERCEL_QUEUES_MANAGED',persistence:'WORKFLOW_EVENT_LOG',parallelism:50,
    currentMonitorRunId:MONITOR_RUN_ID,latestSweepRunId:LATEST_SWEEP_RUN_ID,sweepBootstrap:'ONE_TIME_OPEN',searchProfile:'EU27_LOCALIZED_V2',simulatedWorkersStarted:0,checkedAt:new Date().toISOString(),
  });
  ...
  if(path==='sweep-bootstrap') {
    ...
    return json({ok:true,state:'SWEEP_STARTED',...started,parallelism:50,cycle:1,maxCandidatesPerLane:6,simulatedWorkersStarted:0},202);
  }
