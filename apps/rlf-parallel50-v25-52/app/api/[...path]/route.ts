...
const ONE_SHOT_HASH='ee704c6a0a0727a09947f099263960fe9ec5ae3613d28ee6330ccc1484897b95';
...
async function startSweep(){const id=campaignId('RLF-P50-SWEEP-V255531');const run=await start(parallel50Sweep,[{campaignId:id,cycle:14,maxCandidatesPerLaneCycle:8}]);return{campaignId:id,runId:run.runId};}
async function startMonitor(){const id=campaignId('RLF-P50-MONITOR-V255531');const run=await start(parallel50Campaign,[{campaignId:id,cycles:12,intervalMs:7200000,maxCandidatesPerLaneCycle:6}]);return{campaignId:id,runId:run.runId};}

export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  const parts=(await params).path??[];const path=parts.join('/');void request;
  const base={version:'25.55.31',workflowVersion:'4.8.4_PINNED',searchProfile:'EU27_BING_PRIMARY_ZERO_FALLBACK_V16',delta:'0051',dedupRegistry:CANONICAL_REGISTRY_COVERAGE,identityQuarantineCount:KNOWN_IDENTITY_QUARANTINE_DOMAINS.size,dependencyAudit:{moderate:0,high:0,critical:0,total:0}};
  if(path==='health')return json({ok:true,...base,executionBackend:'VERCEL_WORKFLOW',queue:'VERCEL_QUEUES_MANAGED',persistence:'WORKFLOW_EVENT_LOG',parallelism:50,currentMonitorRunId:MONITOR_RUN_ID,latestSweepRunId:LATEST_SWEEP_RUN_ID,sweepBootstrap:'CLOSED',oneShotSweep:'ARMED',replacementEngineVersion:REPLACEMENT_POLICY.version,replacementSourcePool:REPLACEMENT_POLICY.sourcePool,qaAcceptedNewSuppliers:1,simulatedWorkersStarted:0,checkedAt:new Date().toISOString()});
