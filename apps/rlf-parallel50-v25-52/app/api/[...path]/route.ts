...
const ONE_SHOT_HASH='51deddd3965ffde845d0c55491c8925568841d0b80a4664785f869765bcc3e71';
const MONITOR_RUN_ID='wrun_01M0Q8981CTB61S3DJJ06JZ2FW';
const LATEST_SWEEP_RUN_ID='wrun_01M0TK4XS95SRCRRSG3HFJE162';
...
async function startSweep(){const id=campaignId('RLF-P50-SWEEP-V255521');const run=await start(parallel50Sweep,[{campaignId:id,cycle:11,maxCandidatesPerLaneCycle:8}]);return{campaignId:id,runId:run.runId};}
async function startMonitor(){const id=campaignId('RLF-P50-MONITOR-V255521');const run=await start(parallel50Campaign,[{campaignId:id,cycles:12,intervalMs:7200000,maxCandidatesPerLaneCycle:6}]);return{campaignId:id,runId:run.runId};}

export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  const parts=(await params).path??[];const path=parts.join('/');void request;
  const base={version:'25.55.21',workflowVersion:'4.8.4_PINNED',searchProfile:'EU27_SAME_DOMAIN_EVIDENCE_V12',delta:'0051',dedupRegistry:CANONICAL_REGISTRY_COVERAGE,identityQuarantineCount:KNOWN_IDENTITY_QUARANTINE_DOMAINS.size,dependencyAudit:{moderate:0,high:0,critical:0,total:0}};
