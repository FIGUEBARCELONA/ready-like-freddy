...
const ONE_SHOT_HASH='00c4a0dc2082b68b5f9252c77efed56cf4b70c59cb83fc71ff53a3da5b72f9a4';
const MONITOR_RUN_ID='wrun_01M0Q8981CTB61S3DJJ06JZ2FW';
const LATEST_SWEEP_RUN_ID='wrun_01M0TY7SXWBD74NRWRPZ2H78ZQ';
...
async function startSweep(){const id=campaignId('RLF-P50-SWEEP-V255540');const run=await start(parallel50Sweep,[{campaignId:id,cycle:17,maxCandidatesPerLaneCycle:8}]);return{campaignId:id,runId:run.runId};}
async function startMonitor(){const id=campaignId('RLF-P50-MONITOR-V255540');const run=await start(parallel50Campaign,[{campaignId:id,cycles:12,intervalMs:7200000,maxCandidatesPerLaneCycle:6}]);return{campaignId:id,runId:run.runId};}

export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  const parts=(await params).path??[];const path=parts.join('/');void request;
  const base={version:'25.55.40',workflowVersion:'4.8.4_PINNED',searchProfile:'EU27_COMMONCRAWL_DDG_MULTI_CORPUS_V19',delta:'0051',dedupRegistry:CANONICAL_REGISTRY_COVERAGE,identityQuarantineCount:KNOWN_IDENTITY_QUARANTINE_DOMAINS.size,dependencyAudit:{moderate:0,high:0,critical:0,total:0}};
