...
const CAPABILITY_HASH='39e3bc7c01b9f31d0eee005ced51ad4d7b8ae2a246eb610b608af08bafa986da';
const MONITOR_RUN_ID='wrun_01M0Q8981CTB61S3DJJ06JZ2FW';
const LATEST_SWEEP_RUN_ID='wrun_01M0VKX99Q895W56SVY2EMND9Z';
...
async function startSweep(){const id=campaignId('RLF-P50-SWEEP-V255549');const run=await start(parallel50Sweep,[{campaignId:id,cycle:19,maxCandidatesPerLaneCycle:8}]);return{campaignId:id,runId:run.runId};}

export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  const parts=(await params).path??[];const path=parts.join('/');void request;
  const base={version:'25.55.49',workflowVersion:'4.8.4_PINNED',searchProfile:'EU27_OSM_OVERPASS_BBOX_TILES_V22R2',delta:'0051',dedupRegistry:CANONICAL_REGISTRY_COVERAGE,identityQuarantineCount:KNOWN_IDENTITY_QUARANTINE_DOMAINS.size,dependencyAudit:{moderate:0,high:0,critical:0,total:0}};
  ...
  if(parts[0]==='sweep-once'&&parts[1]){
    if(parts.length!==2||!secureEqual(parts[1],CAPABILITY_HASH))return json({ok:false,code:'NOT_FOUND'},404);
