...
const ONE_SHOT_HASH='e5eb6a78b4fba49c5c0e78f86c45ceb4b1ff38f011fdf8a631fbd1e4aca54998';
...
async function startSweep(){const id=campaignId('RLF-P50-TARGETED-V255554');const run=await start(parallel50Sweep,[{campaignId:id,cycle:20,maxCandidatesPerLaneCycle:1}]);return{campaignId:id,runId:run.runId};}
void parallel50Campaign;

export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  const parts=(await params).path??[];const path=parts.join('/');void request;
  const base={version:'25.55.54',workflowVersion:'4.8.4_PINNED',searchProfile:'EU27_TARGETED_DIRECT_BRAND_VERIFICATION_V23',delta:'0051',dedupRegistry:CANONICAL_REGISTRY_COVERAGE,identityQuarantineCount:KNOWN_IDENTITY_QUARANTINE_DOMAINS.size,dependencyAudit:{moderate:0,high:0,critical:0,total:0}};
