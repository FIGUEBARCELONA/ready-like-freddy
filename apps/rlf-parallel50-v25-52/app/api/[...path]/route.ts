...
async function startSweep() {
  const id=campaignId('RLF-P50-SWEEP');
  const run=await start(parallel50Sweep,[{campaignId:id,cycle:0,maxCandidatesPerLaneCycle:5}]);
...
export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}) {
  ...
  if(parts[0]==='run'&&parts[1]) {
    ...
    return json({
      ok:state.status!=='unavailable',runId:parts[1],status:state.status,activeWorkers:state.active,activeLanes:state.active,
      executionSemantics:['running','pending'].includes(state.status)?'DURABLE_RUNNING_OR_SCHEDULED':'TERMINAL',
      summary:state.output?{...state.output,candidates:undefined}:null,result:state.output,error:state.error,
    },state.status==='unavailable'?404:200);
  }
