...
const ONE_SHOT_HASH='1b9affeb1ad2c83b0a125f39fd0be58e10054c7b41e815710818137a22464b44';
const MONITOR_RUN_ID='wrun_01M0Q8981CTB61S3DJJ06JZ2FW';
const LATEST_SWEEP_RUN_ID='wrun_01M0VK9DCHC8D37Y5P0V63JJS0';
...
async function startSweep(){const id=campaignId('RLF-P50-SWEEP-V255543');const run=await start(parallel50Sweep,[{campaignId:id,cycle:18,maxCandidatesPerLaneCycle:8}]);return{campaignId:id,runId:run.runId};}
async function startMonitor(){const id=campaignId('RLF-P50-MONITOR-V255543');const run=await start(parallel50Campaign,[{campaignId:id,cycles:12,intervalMs:7200000,maxCandidatesPerLaneCycle:6}]);return{campaignId:id,runId:run.runId};}

export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  const parts=(await params).path??[];const path=parts.join('/');void request;
  const base={version:'25.55.43',workflowVersion:'4.8.4_PINNED',searchProfile:'EU27_DDG_ORDER_INDEPENDENT_DIAGNOSTIC_V20',delta:'0051',dedupRegistry:CANONICAL_REGISTRY_COVERAGE,identityQuarantineCount:KNOWN_IDENTITY_QUARANTINE_DOMAINS.size,dependencyAudit:{moderate:0,high:0,critical:0,total:0}};
