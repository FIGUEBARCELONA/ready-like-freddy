...
const ONE_SHOT_SWEEP_HASH='0311d3be1eb9b32abfdc01df1665758dc782d4d8cf5be127b1c6385e6510b10c';
const MONITOR_RUN_ID='wrun_01M0Q8981CTB61S3DJJ06JZ2FW';
const LATEST_SWEEP_RUN_ID='wrun_01M0TAPQKVJYY9QKZPKZ5PPTM5';
...
async function startSweep(){const id=campaignId('RLF-P50-SWEEP-V255512');const run=await start(parallel50Sweep,[{campaignId:id,cycle:8,maxCandidatesPerLaneCycle:8}]);return{campaignId:id,runId:run.runId};}
async function startMonitor(){const id=campaignId('RLF-P50-MONITOR-V255512');const run=await start(parallel50Campaign,[{campaignId:id,cycles:12,intervalMs:7200000,maxCandidatesPerLaneCycle:6}]);return{campaignId:id,runId:run.runId};}

export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  const parts=(await params).path??[];const path=parts.join('/');void request;
  const base={version:'25.55.12',workflowVersion:'4.8.4_PINNED',searchProfile:'EU27_CONTEXTUAL_COUNTRY_IDENTITY_V9',delta:'0051',dedupRegistry:CANONICAL_REGISTRY_COVERAGE,identityQuarantineCount:KNOWN_IDENTITY_QUARANTINE_DOMAINS.size,dependencyAudit:{moderate:0,high:0,critical:0,total:0}};
  if(path==='health')return json({ok:true,...base,executionBackend:'VERCEL_WORKFLOW',queue:'VERCEL_QUEUES_MANAGED',persistence:'WORKFLOW_EVENT_LOG',parallelism:50,currentMonitorRunId:MONITOR_RUN_ID,latestSweepRunId:LATEST_SWEEP_RUN_ID,sweepBootstrap:'CLOSED',oneShotSweep:'ARMED',replacementEngineVersion:REPLACEMENT_POLICY.version,replacementSourcePool:REPLACEMENT_POLICY.sourcePool,qaAcceptedNewSuppliers:0,simulatedWorkersStarted:0,checkedAt:new Date().toISOString()});
  ...
  if(parts[0]==='control'&&parts[1]==='sweep-once'&&parts[2]){
    if(!secureEqual(parts[2],ONE_SHOT_SWEEP_HASH))return json({ok:false,code:'NOT_FOUND'},404);
    const started=await startSweep();
    return json({ok:true,state:'SWEEP_STARTED_ONCE',...started,parallelism:50,simulatedWorkersStarted:0},202);
  }
  if(path==='status'){
