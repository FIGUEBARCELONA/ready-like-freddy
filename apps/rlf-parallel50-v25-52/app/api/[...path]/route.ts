...
const ONE_SHOT_HASH='944bb087b3424a3cbd6b857849fa3bbd0ceef537bb524e1da56b602d6a19e25f';
const MONITOR_RUN_ID='wrun_01M0Q8981CTB61S3DJJ06JZ2FW';
const LATEST_SWEEP_RUN_ID='wrun_01M0TD28K4G1QP7BSPY7A3Z9TF';
...
async function startSweep(){const id=campaignId('RLF-P50-SWEEP-V255518');const run=await start(parallel50Sweep,[{campaignId:id,cycle:10,maxCandidatesPerLaneCycle:8}]);return{campaignId:id,runId:run.runId};}
async function startMonitor(){const id=campaignId('RLF-P50-MONITOR-V255518');const run=await start(parallel50Campaign,[{campaignId:id,cycles:12,intervalMs:7200000,maxCandidatesPerLaneCycle:6}]);return{campaignId:id,runId:run.runId};}

export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  const parts=(await params).path??[];const path=parts.join('/');void request;
  const base={version:'25.55.18',workflowVersion:'4.8.4_PINNED',searchProfile:'EU27_CANONICAL_DEDUP_V11',delta:'0051',dedupRegistry:CANONICAL_REGISTRY_COVERAGE,identityQuarantineCount:KNOWN_IDENTITY_QUARANTINE_DOMAINS.size,dependencyAudit:{moderate:0,high:0,critical:0,total:0}};
