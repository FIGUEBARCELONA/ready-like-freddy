...
const totals = {
  ...
  productCandidateImageReferenceCount: uniqueProductCandidates.reduce(
...
const qualityGatePassed =
  ...
  totals.uniqueProductUrlCount >= 50;
const consolidated = {
  schemaVersion: 3,
  ...
  transportStatus:
    totals.successfulFetchCount > 0 ? "USABLE_SOURCE_CAPTURED" : "NO_USABLE_SOURCE_CAPTURED",
  extractionStatus:
    totals.uniqueProductUrlCount > 0
      ? "PRODUCT_URL_FRONTIER_READY"
      : "NO_PRODUCT_URLS_EXTRACTED",
  qualityGatePassed,
...
await writeFile(
  ...
  uniqueProductCandidates.length
    ? `${uniqueProductCandidates.map(record => JSON.stringify(record)).join("\n")}\n`
    : "",
  "utf8",
);
await writeFile(
