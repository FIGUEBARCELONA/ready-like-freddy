...
if (workerSummaries.some(summary => summary.queueSha256 !== queue.queueSha256)) throw new Error("Worker summary queue hash mismatch");
...
const candidateRecords = records.filter(record => {
  ...
  return Boolean(record.structuredProductEvidence || record.productCode || /\.(?:html?)$/i.test(path));
});
const consolidated = {
  schemaVersion: 1,
  ...
  totals: {
    ...
    duplicateCanonicalGroupCount: [...canonicalGroups.values()].filter(group => group.length > 1).length,
...
await writeFile(`${outDir}/records.ndjson`, `${records.map(record => JSON.stringify(record)).join("\n")}\n`, "utf8");
await writeFile(`${outDir}/product-candidates.ndjson`, `${candidateRecords.map(record => JSON.stringify(record)).join("\n")}\n`, "utf8");
